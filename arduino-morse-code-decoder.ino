...
const int AUTO_MODE_THRESHOLD = 1000;  // If analogRead of SPEED_POT_PIN is below than this number, set auto mode
...
void loop() {
  ...
  if (speed > AUTO_MODE_THRESHOLD) {
    ...
    currentMode = 0;
    dotDuration = map(speed, 0, AUTO_MODE_THRESHOLD - 10, 250, 20);
    if (lastMode != currentMode) {
      lcd.setCursor(0, 0);
      lcd.print(F("Manu: "));
      lastMode = currentMode;
...
void endOfCharacter() {
  ...
  if ((millis() - lastInterruptTime) > minCharacterGap && bufferPosition != 0) {
    ...
    for (byte c = 0; c < 63; c++) {
      ...
      if (currentCharacter[0] == morseCode[c][0] && currentCharacter[1] == morseCode[c][1] && currentCharacter[2] == morseCode[c][2] && currentCharacter[3] == morseCode[c][3] && currentCharacter[4] == morseCode[c][4] && currentCharacter[5] == morseCode[c][5] && currentCharacter[6] == morseCode[c][6] && currentCharacter[7] == morseCode[c][7] && currentCharacter[8] == morseCode[c][8]) {
        // It's a match
